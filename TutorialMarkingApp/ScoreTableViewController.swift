...
class ScoreTableViewController: UITableViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        ...
        if segue.identifier == "ShowEntScoreSegue"
        {
              //down-cast from UIViewController to StudentDetailViewController (this could fail if we didn’t link things up properly)
            if let choosehdScreen = segue.destination as? EnterScoreViewController
                    {
                      choosehdScreen.selectedWeek = selectedWeek!
                    }
            
            //down-cast from UIViewController to StudentDetailViewController (this could fail if we didn’t link things up properly)
            guard let detailViewController = segue.destination as? EnterScoreViewController else
            {
