//
//  GradeHdTableViewController.swift
//  TutorialMarkingApp
//
...
class GradeHdTableViewController: UITableViewController {
    var student : Student?
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        ...
        if segue.identifier == "ShowChooseHdSegue"
        {
              //down-cast from UIViewController to StudentDetailViewController (this could fail if we didn’t link things up properly)
            if let choosehdScreen = segue.destination as? ChooseHdViewController
                    {
                      choosehdScreen.selectedWeek = selectedWeek!
                    }
            
            //down-cast from UIViewController to StudentDetailViewController (this could fail if we didn’t link things up properly)
            guard let detailViewController = segue.destination as? ChooseHdViewController else
            {
