...
class ChooseHdViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    var student : Student?
            ...
            do
            {
                ...
                try db.collection("students").document(student!.id!).updateData(["\(updateScore)": score]){ err in
                    if let err = err {
                        ...
                        if (self.score == 100.0) {
                            self.curLabel.text = "HD+"
                        } else if (self.score == 80.0) {
                            self.curLabel.text = "HD"
                        } else if (self.score == 70.0) {
                            self.curLabel.text = "DN"
                        } else if (self.score == 60.0) {
                            self.curLabel.text = "CR"
                        }else if (self.score == 50.0) {
                            self.curLabel.text = "PP"
                                }else {
                                    self.curLabel.text = "NN"
                                }
                    }
                }
