...
class MultipleTableViewController: UITableViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        ...
        if segue.identifier == "ShowMultipleCheckSegue"
        {
              //down-cast from UIViewController to StudentDetailViewController (this could fail if we didn’t link things up properly)
            if let chooseMumtipleScreen = segue.destination as? ChooseMultipleViewController
                    {
                chooseMumtipleScreen.selectedWeek = selectedWeek!
                    }
            
            //down-cast from UIViewController to StudentDetailViewController (this could fail if we didn’t link things up properly)
            guard let detailViewController = segue.destination as? ChooseMultipleViewController else
            {
